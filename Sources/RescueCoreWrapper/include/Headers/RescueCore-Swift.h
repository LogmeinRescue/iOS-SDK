...
#ifndef RESCUECORE_SWIFT_H
#define RESCUECORE_SWIFT_H
...
/// Notify the delegate that the broadcast extension stopped
- (void)rescueBroadcastDidStop;
/// Notify the delegate about session status change
- (void)rescueBroadcastSessionStatusDidChange:(enum RescueSessionStatus)status;
@end
...
typedef SWIFT_ENUM(NSUInteger, RescueCameraDevice, open) {
...
/// Flashlight state.
typedef SWIFT_ENUM(NSUInteger, RescueCameraFlash, open) {
...
typedef SWIFT_ENUM(NSUInteger, RescueChatMessageType, open) {
...
typedef SWIFT_ENUM(NSUInteger, RescueDisplayMode, open) {
...
/// Set this property to <em>true</em> before you invoke <em>startSession()</em>, if you would like to disable notifications.
@property (nonatomic) BOOL disableNotifications;
...
- (void)endSession;
/// End the Rescue broadcast session
