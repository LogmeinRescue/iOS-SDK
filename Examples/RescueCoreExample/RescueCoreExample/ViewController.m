...
@interface ViewController ()  <RescueSessionDelegate, RescueChatDelegate, UITextFieldDelegate>
...
- (void)startRescueSession
{
    ...
    [[RescueDisplay sharedInstance] enable];

    // enable/disable whiteboard and laser pointer for the session
    [RescueDisplay sharedInstance].disableLaserPointer = [[NSUserDefaults standardUserDefaults] boolForKey:kDisableLaserPointerDefaultsKey];
    [RescueDisplay sharedInstance].disableWhiteboard = [[NSUserDefaults standardUserDefaults] boolForKey:kDisableWhiteboardDefaultsKey];
    
    // set hidden view for the session (the chat input field)
    [[RescueDisplay sharedInstance] setView:self.chatTextField hidden:[[NSUserDefaults standardUserDefaults] boolForKey:kHideChatFieldDefaultsKey]];
    
    // set screen sharing mode for the session
    switch ([[NSUserDefaults standardUserDefaults] integerForKey:kScreenShareModeDefaultsKey]) {
        case RescueDisplayModeAppScreen:
            // share the screen
            [RescueDisplay sharedInstance].mode = RescueDisplayModeAppScreen;
            break;
            
        case RescueDisplayModeRootView:
            // share only party of the app (chat log)
            [RescueDisplay sharedInstance].mode = RescueDisplayModeRootView;
            [RescueDisplay sharedInstance].rootView = self.textView;
            break;
            
        default:
            // disable screen sharing
            [RescueDisplay sharedInstance].mode = RescueDisplayModeDisabled;
            break;
    }
    ...
    switch ([[NSUserDefaults standardUserDefaults] integerForKey:kSessionStartModeDefaultsKey]) {
        ...
        default:
        {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Set Pin Code", @"")
                                                                                     message:NSLocalizedString(@"Please enter your pin code.", @"")
                                                                              preferredStyle:UIAlertControllerStyleAlert];

            [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
                textField.keyboardType = UIKeyboardTypeNumberPad;
                textField.placeholder = NSLocalizedString(@"pin code", @"");
            }];
...
- (void)rescueSessionStatusDidChange:(RescueSessionStatus)status
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        {
            self.navigationItem.rightBarButtonItem.title = NSLocalizedString(@"Disconnect", @"");
        ...
        {
            self.navigationItem.rightBarButtonItem.title = NSLocalizedString(@"Connect", @"");
            self.navigationItem.rightBarButtonItem.enabled = YES;
